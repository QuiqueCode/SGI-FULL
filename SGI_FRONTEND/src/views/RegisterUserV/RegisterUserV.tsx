...
import "./RegisterUserVStyle.css";
import { RegisterUserVM } from "../../viewModels/RegisterUserVM/RegisterUserVM";
import { AsignRolVM } from "../../viewModels/AsignRolVM/AsignRolVM";

export default function RegisterUserV() {
  const { goBack,state,changeState, formData,handleInputChange, getDepartament, department,setRol,handleSubmit } = RegisterUserVM();
  const {  getRoles,
    roles, }= AsignRolVM();
...
{roles.map((rol,index)=>(
   <IonList inset={true} key={index}>
       <IonItem detail={false}>
         ...
         <IonButton className="asign" disabled={formData.ROLES.includes(rol.CN_ID_ROL) ? true : false}  onClick={()=>{setRol(rol.CN_ID_ROL)}} > {formData.ROLES.includes(rol.CN_ID_ROL) ? 'Asignado' : 'Asignar'}</IonButton>
       </IonItem>
     </IonList>
